 ...
 * <p>四条行为契约:
 * <ol>
 *   <li>不缓存(每次重新读文件/查库/调 API)</li>
 *   <li>核心记忆区永不被截断,截断只作用在归档区</li>
 *   <li>写核心还是写归档由 Agent 经 scope 显式指定,系统不猜</li>
 *   <li>recall 是关键词检索不做复杂化</li>
 * </ol>
 *
 * <p>三档实现:MarkdownMemoryStore(默认) / SqliteMemoryStore / Mem0MemoryStore,
 * 靠配置 memory.backend 选一个,换后端只改一行配置,上层不动。
 */

...
/**
 * 核心阶段唯一实现。用通用 HTTP webhook 承接所有场景——
 * 企业微信、飞书、钉钉的群机器人都提供 webhook 地址,核心阶段不逐家接专用 API。
 *
 * <p>发送前过 Sandbox.enforce(HTTP_REQUEST, url) 域名白名单校验,
 * 跟 http_post 共享同一份 http.allowed_domains 配置,不新增 Sandbox 逻辑。
 */
@Component

package com.oryxos.core.mcp;

import java.util.List;

/**
 * MCP server 的管理台契约(依赖倒置):web 只认这个接口,具体实现(读写 {@code mcp_servers.yaml}、维护连接、把工具注册进 {@code
 * ToolRegistry})留给 tool——同 {@code SandboxWhitelistPort} 的既有分层方式(web 不反向依赖 tool)。
 *
 * <p>增/改/删都是"持久化 + 即时生效":加一个就立刻尝试连接并把它的工具注册进共享 {@code ToolRegistry},删一个就立刻断开并注销它的工具,不需要重启 OryxOS(第
 * 30 节 WorkspaceWatcher 之于 Agent 目录的同款设计取向)。
 */

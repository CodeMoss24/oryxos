...
/**
 * Agent 的核心循环引擎。输入 Session 和用户消息,输出最终响应。 内部维护当前迭代次数,调用 ProviderService 调 LLM,调用 ToolExecutor 执行
 * Tool。
 *
 * <p>核心算法(Reason + Act):
 *
 ...
 * </pre>
 *
 * <p>注意:不依赖 Spring AI 的 Agent 抽象,自实现;Spring AI 只用协议转换 + schema 生成, 禁用其自动 tool 执行,否则 tool 会被调两次。
 */
@Component
public class ReActLoop {
  ...
  public String run(Session session, String userMessage, Profile profile, String agentMdBody) {
    ...
    log.warn(
        "ReAct loop reached MAX_ITERATIONS={} for session {}",
        maxIterations,

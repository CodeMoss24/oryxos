 ...
 * <ol>
 *   <li>把当前 Profile 放进 ProfileContext(ThreadLocal,虚拟线程下每个请求天然独立)</li>
 *   <li>调 ReActLoop.run 跑完循环</li>
 *   <li>持久化 Session(扩展阶段实现)</li>
 *   <li>finally 里清掉 ProfileContext</li>
 * </ol>
 *
...
public class AgentService {
    ...
    public String process(Session session, String userMessage) {
        Profile profile = profileRegistry.find(session.getProfileName())
                .orElseThrow(() -> new IllegalStateException(
                        "Profile not found: " + session.getProfileName()));
        try {
            ProfileContext.set(profile);
            String agentMdBody = loadAgentMdBody(profile.getName());
            return reActLoop.run(session, userMessage, profile, agentMdBody);
        } finally {
    ...
    private String loadAgentMdBody(String profileName) {
        try {
            java.nio.file.Path agentMd = java.nio.file.Path.of(".oryxos", "agents", profileName, "AGENT.md");

...
/**
 * 第三种触发源(钟推)。基于 Spring 的 ThreadPoolTaskScheduler + CronTrigger 动态注册任务,
 * 不用静态的 @Scheduled 注解(触发规则要按 Profile 配置动态生成)。
 *
 * <p>并发控制:每个定时任务用一把进程内的 ReentrantLock(按任务 id 维度)防止同一任务重叠执行。
 * 核心阶段是单实例部署,这把锁只解决"同一进程内不重叠",不是分布式锁。
 *
 * <p>会话身份:钟推也要落 Session,channel 和 user 都固定为 "scheduler",
 * 同一个 Profile 的历次定时触发复用同一个 Session。
 */
@Component
public class AgentScheduler {
    ...
    private void registerOne(String profileName, Schedule schedule) {
        ZoneId zone = schedule.zone() != null
        ...
        ScheduledFuture<?> future = taskScheduler.schedule(
                () -> runOnce(profileName, schedule.message(), lock),
                trigger);

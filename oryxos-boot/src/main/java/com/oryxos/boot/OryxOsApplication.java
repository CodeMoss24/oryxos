 ...
 *
 * <p>main 里按命令分流(CLAUDE.md 九):不需要 Spring 上下文的"轻命令"直接走 Picocli 执行并 return,
 * 不起 Spring;需要 LLM/HTTP/调度的"重命令"才 SpringApplication.run,启动后由 CommandLineRunner 派发。
 *
 * <p>轻命令集:init / status / profile / provider / tool / session 以及无参数(打印 usage)。
 * 重命令集:chat / serve / gateway。
 */
...
public class OryxOsApplication implements CommandLineRunner {
    ...
    private static final Set<String> LIGHT_COMMANDS =
            Set.of("init", "status", "profile", "provider", "tool", "session",
                    "-h", "--help", "-V", "--version");
    ...
    public OryxOsApplication(CommandLine commandLine,

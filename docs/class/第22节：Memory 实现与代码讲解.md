...
![Memory 架构：MemoryService 门面统一收口，长期记忆走可插拔的 LongTermMemoryStore（md/sqlite/mem0）](images/class-22-1.svg)
...
![核心记忆区与归档记忆区：截断只作用在归档区，核心区永远完整（三档后端共同遵守的契约二）](images/class-22-2.svg)
...
![无缓存读写验证：save_memory 写入后 recall_memory 立刻可见（契约一，三档后端各自验证）](images/class-22-3.svg)

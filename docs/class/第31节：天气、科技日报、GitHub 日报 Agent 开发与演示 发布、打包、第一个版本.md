...
![天气 Agent 端到端：定时触发、查天气、生成建议、推送](images/class-31-1.svg)
...
![日报 Agent 端到端：定时触发、read_file 读子指令、注入记忆、拉新闻、组稿、推送](images/class-31-2.svg)
...
![GitHub 日报 Agent 端到端：触发 → 读 AGENT.md 正文 → shell 跑脚本抓数据 → 组稿推送](images/class-31-4.svg)
...
![发布产物：一个 fat JAR + 一个 .oryxos 工作区（agents 目录）](images/class-31-3.svg)
